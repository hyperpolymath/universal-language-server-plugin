...
[arabic]
. *Email*: Send security reports to security@universal-connector.org (if
available) or create a private security advisory on GitHub
. *Include*:
...
*In Scope:* - LSP server vulnerabilities (command injection, memory
safety) - HTTP API vulnerabilities (authentication bypass, injection
attacks) - WebSocket vulnerabilities (message injection, DoS) -
Dependency vulnerabilities - Build process security issues

*Out of Scope:* - Editor client vulnerabilities (responsibility of
editor maintainers) - Denial of service requiring unrealistic resources
